...
function expandBoundaryPointSegment(from: RoutePoint, to: RoutePoint) {
  const inserted = getFirBoundaryPath(from, to)
  if (inserted.length === 0) {
    return []
  }

  const directLengthNm = distanceNm(from, to)
  const boundaryLengthNm = boundaryPathLength([from, ...inserted, to])

  return boundaryLengthNm > directLengthNm * 1.08 ? inserted : []
}

function expandGeoJsonRing(ring: number[][]) {
  if (ring.length < 2) {
    return ring
  }
  ...
  for (let index = 0; index < ring.length - 1; index += 1) {
    ...
    const inserted = expandBoundaryPointSegment(
      { lat: current[1], lon: current[0] },
      { lat: next[1], lon: next[0] },
    )

    inserted.forEach((point) => expanded.push([point.lon, point.lat]))
  }

  expanded.push(ring[ring.length - 1])
  return expanded.length > ring.length ? expanded : ring
}

export function expandSwedenFirBoundaryGeometry<T extends { type: string; coordinates: unknown }>(geometry: T): T {
  if (geometry.type === 'Polygon' && Array.isArray(geometry.coordinates)) {
    return {
      ...geometry,
      coordinates: (geometry.coordinates as number[][][]).map(expandGeoJsonRing),
    }
  }
  ...
  if (geometry.type === 'MultiPolygon' && Array.isArray(geometry.coordinates)) {
    return {
      ...geometry,
      coordinates: (geometry.coordinates as number[][][][]).map((polygon) => polygon.map(expandGeoJsonRing)),
    }
  }
